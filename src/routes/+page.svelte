<script lang="ts">
    ...
    $: channelList = telegramChannelLists.get(activeUser.id) || [];
    ...
    <div class="flex flex-row">
        <div class="basis-1/4">
            <ul class="menu bg-base-100 p-2 rounded-box ">
                {#each users as user (user.id)}
                    <li>
                        <a href={null} class={activeUser.id === user.id ? "active" : ""} on:click={() => handleUserClick(user)}>
        ...
        <div class="basis-3/4">
            <div class="mask mask-circle w-12">
                <img src="/img/profiles/tg/{activeUser.id}.png" alt="" />
            </div>
            {activeUser.name} 알림 채널

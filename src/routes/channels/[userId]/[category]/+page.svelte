...
<div class="sm:flex mb-4">
    ...
    <div class="flex-none tabs tabs-bordered mt-2.5 mr-2 touch-manipulation">
    ...
    <div class="card w-full bg-base-200">
        <div class="card-body p-4 sm:p-6 gap-3">
            ...
            <div class="card-actions justify-end">
                {#if channel.isPublic}
                <a class="btn btn-neutral min-h-0 h-10 px-3" href={getPreviewLink(channel)} target="_blank" rel="noopener noreferrer">
                    <GlobeIcon class="w-5 h-5" />미리보기
                </a>
                {/if}
                <a class="btn btn-primary min-h-0 h-10 " href={getDirectLink(channel)}>
                    <TelegramIcon class="w-5 h-5" />열기
                </a>
            </div>
